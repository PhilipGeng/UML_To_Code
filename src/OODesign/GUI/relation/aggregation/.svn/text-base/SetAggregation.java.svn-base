...
public class SetAggregation extends JFrame {
	JavaClass classA;
	JavaClass classB;
	String aggResult;
	...
	public SetAggregation(){
		...
		promptLabel = new JLabel("set Class A has a Class B (aggregation)");
		...
		classBConfirmButton.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				aggResult = classA.getName()+" has a "+classB.getName();
				resultLabel.setText(aggResult);
			}		
		});
